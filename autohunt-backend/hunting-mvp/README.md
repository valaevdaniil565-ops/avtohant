...
### Reuse The Live Bot At `C:\hunting-mvp`

If your working Telegram collector/bot already lives in `C:\hunting-mvp`, this monorepo backend can attach to the same Postgres automatically.

- If `autohunt-backend/hunting-mvp/.env` does not exist, the backend falls back to `C:\hunting-mvp\.env`.
- You can override that autodetection with process env variables `HUNTING_MVP_ENV_FILE` or `HUNTING_MVP_EXTERNAL_DIR`.
- This lets the web API read the same `vacancies`, `specialists`, `sources` and `matches` tables that the live bot keeps updating.

In practice that means new Telegram vacancies and existing bench records will show up in the web project without a second import pipeline.
