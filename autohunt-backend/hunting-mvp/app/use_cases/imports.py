...
def _extract_resume_url(structured_fields: dict[str, Any] | None, raw_text: str) -> str:
    if structured_fields:
        for key in ("resume_url", "cv_url", "portfolio_url"):
            value = str(structured_fields.get(key) or "").strip()
            if value:
                return value
    urls = extract_external_urls(raw_text)
    non_telegram = [url for url in urls if "t.me/" not in url.lower()]
    return non_telegram[0] if non_telegram else ""


def _load_external_resume_text(
    source_fetcher: MCPSourceFetcherClient | None,
    resume_url: str,
    cache: dict[str, str],
) -> str:
    normalized_url = str(resume_url or "").strip()
    if not source_fetcher or not normalized_url:
        return ""
    cached = cache.get(normalized_url)
    if cached is not None:
        return cached
    try:
        result = source_fetcher.fetch_url(normalized_url)
    except Exception:
    ...
    if not result.ok:
        cache[normalized_url] = ""
        return ""
    parts = [str(item.text or "").strip() for item in result.items if str(item.text or "").strip()]
    cache[normalized_url] = "\n\n".join(parts[:8]).strip()
    ...
    *,
    ...
    for unit_idx, unit in enumerate(units, start=1):
        ...
        if mtype in ("BENCH", "BENCH_LIST"):
            enriched_raw_unit_text = raw_unit_text
            resume_url = _extract_resume_url(structured_fields, raw_unit_text)
            resume_text = _load_external_resume_text(source_fetcher, resume_url, resume_cache)
            if resume_text:
                enriched_raw_unit_text = _compose_external_text(raw_unit_text, resume_text, resume_url, "resume", None)
            if structured_fields and unit_entity_hint == "BENCH":
                items = [extraction_use_cases.build_structured_specialist_item(structured_fields, enriched_raw_unit_text)]
            else:
                data = extraction_use_cases.safe_json_loads(
            ...
            for idx, item in enumerate(items, start=1):
                is_available = resolve_specialist_is_available(item, enriched_raw_unit_text)
                ...
                search_text = build_search_text(item)
                embedding = ollama.embed(search_text)
                specialist_id = repo.upsert_specialist(item, enriched_raw_unit_text, embedding, status)
                source_trace_use_cases.insert_source(
                    engine,
                    ...
                    sender_name=sender_name,
                    message_url=canonical_url,
                    raw_text=enriched_raw_unit_text,
                    idx=(unit_idx * 1000 + idx),
                    source_type=str(unit.get("source_type") or "web_api"),
                    external_url=unit.get("external_url"),
                    external_kind=unit.get("external_kind"),
