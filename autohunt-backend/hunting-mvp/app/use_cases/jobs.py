...
def execute_job(engine, job: JobRecord, *, repo: Repo, ollama: OllamaClient, source_fetcher: MCPSourceFetcherClient | None = None) -> dict[str, Any]:
    ...
    if job.kind == "partner_companies_sync":
        ...
        extracted_entries = extract_partner_company_sources_from_sheet(source_url, source_fetcher) if source_url else []
        discovered_source_urls = [str(entry.get("source_url") or "").strip() for entry in extracted_entries]
        unique_source_urls: list[str] = []
        seen_urls: set[str] = set()
        for candidate_url in [*discovered_source_urls, *extra_source_urls]:
            normalized_url = str(candidate_url or "").strip()
            if not normalized_url or normalized_url in seen_urls:
                continue
            seen_urls.add(normalized_url)
            unique_source_urls.append(normalized_url)
        if unique_source_urls:
            set_json_list_setting(engine, PARTNER_BENCH_SOURCE_URLS_KEY, unique_source_urls)
        upsert_partner_company_mentions(engine, counts, source_url=source_url)
        processed_sources: list[str] = []
        imported_specialists = 0
        imported_errors: list[str] = []
        for index, partner_url in enumerate(unique_source_urls, start=1):
            try:
                summary = import_use_cases.process_url_import(
                    f"{job.id}-partner-{index}",
                    engine=engine,
                    repo=repo,
                    ollama=ollama,
                    source_fetcher=source_fetcher,
                    url=partner_url,
                    forced_type="BENCH",
                    bench_origin="partner",
                )
                processed_sources.append(partner_url)
                imported_specialists += int(summary.specialists or 0)
                imported_errors.extend(summary.errors)
            except Exception as exc:
                imported_errors.append(f"{partner_url}: {type(exc).__name__}: {exc}")
        return {
            "source_url": source_url,
            "source_urls": unique_source_urls,
            "sources_processed": len(processed_sources),
            "sources_discovered": len(unique_source_urls),
            "companies_synced": len(counts),
            "counts": counts,
            "specialists_imported": imported_specialists,
            "errors": imported_errors[:200],
        }
...
def schedule_interval_jobs(engine) -> list[JobRecord]:
    ...
    partner_source_url = get_effective_setting(
        engine,
        PARTNER_COMPANIES_SOURCE_URL_KEY,
