...
def generate_specialist_synthetic_id(data: dict[str, Any]) -> str:
    resume_url = str(data.get("resume_url") or data.get("cv_url") or "").strip().lower()
    if not resume_url:
        ...
        if isinstance(source_urls, list):
            for value in source_urls:
                ...
                if candidate:
                    ...
                    break
    if resume_url:
        return hashlib.sha256(f"specialist|resume|{resume_url}".encode("utf-8")).hexdigest()[:16]

    name = re.sub(r"\s+", " ", str(data.get("name") or "").strip().lower())
    role = re.sub(r"\s+", " ", str(data.get("role") or "").strip().lower())
    location = re.sub(r"\s+", " ", str(data.get("location") or "").strip().lower())
    stack = sorted([str(s).strip().lower() for s in (data.get("stack") or []) if str(s).strip()])
    if name:
        normalized = f"specialist|name|{name}|{role}|{location}|{stack}"
        return hashlib.sha256(normalized.encode("utf-8")).hexdigest()[:16]

    rate_hint = data.get("rate_min") or data.get("rate_max")
    return generate_synthetic_id(str(data.get("role") or ""), list(data.get("stack") or []), data.get("grade"), rate_hint)
...
class Repo:
    ...
    def upsert_specialist(
        ...
        if data.get("experience_years_min") == data.get("experience_years_max"):
            exp = data.get("experience_years_min")
        syn = generate_specialist_synthetic_id(data)
        ...
        q = text(
            ...
            ON CONFLICT(synthetic_id) DO UPDATE
              SET role = EXCLUDED.role,
                  ...
                  hired_at = CASE WHEN CAST(:status AS varchar)='hired' THEN NOW() ELSE NULL END,
                  updated_at = NOW(),
