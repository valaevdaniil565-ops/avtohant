...
class TelethonCollector:
    ...
    async def _load_all_channel_dialogs(self) -> tuple[list[ChannelRow], int]:
        ...
        existing_by_id = {row.telegram_id: row for row in existing_rows}
        ...
        async for dialog in self.client.iter_dialogs():
            ...
            existing = existing_by_id.get(telegram_id) or existing_by_username.get(_normalize_username(username))
            channels.append(
                ChannelRow(
                    ...
                    source_kind=(existing.source_kind if existing else self._detect_source_kind(entity)),
                )
            )

        for row in existing_rows:
            if row.telegram_id in seen_ids:
                continue

            username = _normalize_username(row.username)
            if self._is_bot_source(row.telegram_id, username):
                dropped += 1
                continue
            if self.s.archive_enabled and self._is_archive_source(row.telegram_id, username):
                dropped += 1
                continue

            lookup: Any = username or row.telegram_id
            try:
                entity = await self.client.get_entity(lookup)
            except Exception as exc:
                log.warning(
                    "Failed to resolve DB Telegram channel telegram_id=%s username=%s: %s",
                    row.telegram_id,
                    username,
                    exc,
                )
            ...
            resolved_id = int(getattr(entity, "id", 0) or row.telegram_id or 0)
            if bool(getattr(entity, "broadcast", False)) or bool(getattr(entity, "megagroup", False)):
                telegram_id = _botapi_channel_id_from_telethon_entity_id(resolved_id)
            else:
            ...
            entity_username = str(entity_username) if entity_username else username
            title = getattr(entity, "title", None) or entity_username or row.title or f"Source {telegram_id}"
            title = str(title)
            ...
            channels.append(
                ChannelRow(
                    ...
                    source_kind=row.source_kind or self._detect_source_kind(entity),
                )
            )
