...
from backend.app.api.schemas import ImportJobAcceptedResponse, ImportJobListResponse, ImportJobStatusResponse, ImportSummaryResponse, TextImportRequest, UrlImportRequest
...
def import_text(payload: TextImportRequest) -> ImportJobAcceptedResponse:
    engine = get_engine()
    job = enqueue_text_import(engine, text_value=payload.text, forced_type=payload.forced_type)
    write_audit_event(
        engine,
        event_type="import.text.enqueued",
        entity_type="job",
        entity_id=job.id,
        payload={"forced_type": payload.forced_type},
    )
...
def import_url(payload: UrlImportRequest) -> ImportJobAcceptedResponse:
    engine = get_engine()
    job = enqueue_url_import(engine, url=payload.url, forced_type=payload.forced_type)
    write_audit_event(
        engine,
        event_type="import.url.enqueued",
        entity_type="job",
        entity_id=job.id,
        payload={"url": payload.url, "forced_type": payload.forced_type},
    )
    return ImportJobAcceptedResponse(job_id=job.id, status=job.status)
...
async def import_file(
    ...
    write_audit_event(
        ...
        entity_type="job",
        entity_id=job.id,
        payload={"filename": str(file.filename or "upload.bin"), "forced_type": forced_type},
    )
    return ImportJobAcceptedResponse(job_id=job.id, status=job.status)
