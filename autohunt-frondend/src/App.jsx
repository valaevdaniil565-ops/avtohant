...
function App() {
  ...
  const [runRate, setRunRate] = React.useState("");
  ...
  const handleRunProcess = async () => {
    try {
      const result = await api.runManualMatch({
        ...
        text: runText,
        limit: 10,
        rate: runRate ? Number(runRate) : undefined
      });
  ...
  const handleRowAction = (kind, payload) => {
    ...
    const cvUrl = pickActualSpecialistLink(
      meta.source_meta?.structured_fields?.resume_url,
      meta.source_meta?.row_map?.resume_url,
      meta.source_display,
      meta.original_text,
      meta.source_url
    );
    ...
    const dialogByKind = {
      ...
      vacancies: {
        title: `Вакансия: ${payload[1] ?? "—"}`,
        rows: [
          ["Владелец чата", payload[0]],
          ["Специалист", payload[2]],
          ["Грейд", payload[3]],
          ["Ставка", payload[4]],
          ["Статус", payload[5]],
          ["Дата публикации", payload[6]],
...
function VacanciesPage({ items, total, onRefresh, onSearch, onOpen }) {
  return (
    <section className="page">
      ...
      <FilterBar fields={["Владелец чата", "Вакансия", "Специалист", "Грейд", "Статус"]} buttonLabel="Искать" onSubmit={onSearch} />
      <Table
        columns={["Владелец чата", "Вакансия", "Специалист", "Грейд", "Ставка", "Статус", "Дата публикации", ""]}
        rows={items.map((item) => [
          <div key={`owner-${item[1]}`} className="cell-subtitle">{item[0]}</div>,
          <div key={`vacancy-${item[1]}`} className="cell-title">{item[1]}</div>,
          <div key={`specialist-${item[1]}`} className="cell-subtitle">{item[2]}</div>,
          <Badge key={`grade-${item[1]}`} tone={item[3] === "Senior" ? "purple" : item[3] === "Junior" ? "green" : "blue"}>
            {item[3]}
          </Badge>,
          item[4],
          <Badge key={`status-${item[1]}`} tone="green">{item[5]}</Badge>,
          item[6],
          <button key={`details-${item[1]}`} className="table-action" type="button" onClick={() => onOpen(item)} title="Показать детали">
            ⋯
          </button>
        ])}
...
function BenchPage({
  ...
  return (
    <section className="page">
      ...
      <FilterBar fields={["ФИО", "Специалист", "Грейд", "Статус"]} buttonLabel="Искать" onSubmit={onSearch} />
      <Panel title="Логика подбора" subtitle="">
        <div className="priority-note">
          В таблице оставлены только ключевые поля для быстрого просмотра бенча: ФИО, специализация, грейд и статус.
        </div>
      </Panel>
      <Table
        columns={["ФИО", "Специалист", "Грейд", "Статус", ""]}
        rows={items.map((item) => [
          <div key={`person-${item[0]}`} className="person-cell">
            <div className="avatar-mini">{item[0][0]}</div>
            <span>{item[0]}</span>
          </div>,
          <div key={`role-${item[0]}`} className="cell-title">{item[1]}</div>,
          <Badge key={`grade-${item[0]}`} tone={item[2] === "Lead" ? "gold" : item[2] === "Senior" ? "purple" : "blue"}>
            {item[2]}
          </Badge>,
          <Badge key={`status-${item[0]}`} tone="green">{item[3]}</Badge>,
...
function RunPage({ mode, value, rate, result, onModeChange, onChange, onRateChange, onClear, onProcess }) {
  return (
    <section className="page">
      ...
      <div className="split-grid">
        <Panel title="Входные данные" subtitle="">
          ...
          <label className="field field--compact">
            <span>{mode === "bench" ? "Ставка специалиста" : "Бюджет вакансии"}</span>
            <input
              value={rate}
              inputMode="numeric"
              onChange={(event) => onRateChange(event.target.value.replace(/[^\d]/g, ""))}
              placeholder={mode === "bench" ? "Например, 2200" : "Например, 350000"}
            />
            <small>Это отдельный фильтр под полем текста, чтобы вручную проверять релевантность по ставке.</small>
          </label>
                      ...
                      <article key={`${section.id}-${item.id}`} className="run-result-item">
                        <div className="run-result-item__head">
                          <div>
                            ...
                            {item.subtitle ? <div className="cell-subtitle">{item.subtitle}</div> : null}
                          </div>
                        ...
                        {item.source_url ? (
                          <div className="run-result-item__meta">
                            <a href={item.source_url} target="_blank" rel="noreferrer">
                              {mode === "vacancy" ? "Открыть CV / профиль" : "Открыть вакансию"}
                            </a>
                          </div>
                        ) : null}
